...
namelist = ["Alice", "Bob", "Charlie", "Dave", "Emily", "Frank", "Grace", "Hannah", "Isabel", "Jake"]

class filteredlist(tk.Tk):
    def __init__(self):
        tk.Tk.__init__(self)
        ...
        frame2.grid(row=2, column=1, columnspan=1, sticky='we')

        # Put the filter label and entry box in the frame.
        tk.Label(frame, text='Filter:').pack(side='left')
        tk.Button(frame2, text='Close', command=self.destroy).pack(side='right')
        tk.Button(frame2, text='Select', command=self.select).pack(side='left')
        self.selectedlabel = tk.Label(frame, text='selected name:')
        self.selectedlabel.pack(side='right')
    ...
    def select(self):
        selected_name_i = self.listbox.curselection()
        self.selectedlabel.configure(text=f'selected name: {self.listbox.get(selected_name_i)}')
